...
using T24_03_01_InheritanceMiniProject.Models;

namespace T24_03_01_InheritanceMiniProject
{
    internal class Program
    {
        static void Main(string[] args)
        {
            List<IRenatable> renatables = new List<IRenatable>();
            List<IPurchasable> purchasables = new List<IPurchasable>();

            var vehicle = new VehicleModel { DealerFee = 25, ProductName = "Kia Optima" };
            var book = new BookModel { NumberOfPages = 350, ProductName = "A Tale of two Cities" };
            var excavator = new ExcavatorModel { ProductName = "Buldozer", QuantityInStock = 2 };

            renatables.Add(vehicle);
            renatables.Add(excavator);
            ...
            {
                foreach (var item in renatables)
                {
